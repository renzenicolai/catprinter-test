...
#if MYNEWT_VAL(BLE_INCL_SVC_DISCOVERY) || MYNEWT_VAL(BLE_GATT_CACHING_INCLUDE_SERVICES)
static int  peer_inc_disced(uint16_t conn_handle, const struct ble_gatt_error* error,
    ...
    SLIST_FOREACH(svc, &peer->svcs, next) {
        SLIST_FOREACH(chr, &svc->chrs, next) {
            if (!chr_is_empty(svc, chr) && SLIST_EMPTY(&chr->dscs) && peer->disc_prev_chr_val <= chr->chr.def_handle) {
                ...
                rc = ble_gattc_disc_all_dscs(peer->conn_handle, chr->chr.val_handle, chr_end_handle(svc, chr),
...
static int peer_chr_disced(uint16_t conn_handle, const struct ble_gatt_error* error, const struct ble_gatt_chr* chr,
                           void* arg) {
    struct peer* peer;
    int          rc;
...
static void peer_disc_chrs(struct peer* peer) {
    ...
    SLIST_FOREACH(svc, &peer->svcs, next) {
        if (!peer_svc_is_empty(svc) && SLIST_EMPTY(&svc->chrs)) {
            peer->cur_svc = svc;
            rc = ble_gattc_disc_all_chrs(peer->conn_handle, svc->svc.start_handle, svc->svc.end_handle, peer_chr_disced,
                                         peer);
            if (rc != 0) {
                peer_disc_complete(peer, rc);
            }
...
#if (MYNEWT_VAL(BLE_INCL_SVC_DISCOVERY) || MYNEWT_VAL(BLE_GATT_CACHING_INCLUDE_SERVICES))
static struct peer_incl_svc* peer_incl_svc_find_prev(const struct peer_svc* svc, uint16_t incl_svc_handle) {
...
static struct peer_incl_svc* peer_incl_svc_find(const struct peer_svc* svc, uint16_t incl_svc_handle,
...
static int peer_inc_add(struct peer* peer, uint16_t svc_start_handle, const struct ble_gatt_incl_svc* gatt_incl_svc) {
    struct peer_incl_svc* incl_svc;
...
static void peer_disc_incs(struct peer* peer) {
    ...
    if (svc != NULL && !peer_svc_is_empty(svc)) {
        rc = ble_gattc_find_inc_svcs(peer->conn_handle, svc->svc.start_handle, svc->svc.end_handle, peer_inc_disced,
                                     peer);
        if (rc != 0) {
            peer_disc_chrs(peer);
        }
...
void peer_list_all(const struct peer* peer) {
    ...
    SLIST_FOREACH(svc, &peer->svcs, next) {
        ...
        SLIST_FOREACH(chr, &svc->chrs, next) {
            ...
            printf("    Characteristic def_handle=%d val_handle=%d properties=0x%02x uuid=%s\r\n", chr->chr.def_handle,
                   chr->chr.val_handle, chr->chr.properties, buffer);
            SLIST_FOREACH(dsc, &chr->dscs, next) {
                ble_uuid_to_str((ble_uuid_t*)&dsc->dsc.uuid, buffer);
                printf("      Descriptor handle=%d uuid=%s\r\n", dsc->dsc.handle, buffer);
            }
        }
    }
}
...
const struct peer_chr* peer_chr_find_uuid(const struct peer* peer, const ble_uuid_t* svc_uuid,
    ...
    SLIST_FOREACH(chr, &svc->chrs, next) {
        ...
        ble_uuid_to_str((ble_uuid_t*)&chr->chr.uuid, buffer);
        // printf("peer_chr_find_uuid: has not matching characteristic with UUID %s\r\n", buffer);
    }
...
const struct peer_dsc* peer_dsc_find_uuid(const struct peer* peer, const ble_uuid_t* svc_uuid,
    ...
    SLIST_FOREACH(dsc, &chr->dscs, next) {
        ...
        ble_uuid_to_str((ble_uuid_t*)&dsc->dsc.uuid, buffer);
        // printf("peer_dsc_find_uuid: has not matching descriptor with UUID %s\r\n", buffer);
    }
...
#endif
{
    ...
    rc = os_mempool_init(&peer_incl_svc_pool, max_incl_svcs, sizeof(struct peer_incl_svc), peer_incl_svc_mem,
                         "peer_incl_svc_pool");
    if (rc != 0) {
        rc = BLE_HS_EOS;
        goto err;
